...
function getAbilityModifier(integerScore) { //use an abilityScore element as the argument
    return Math.floor((integerScore - 10)/2);
}

function getAbilityModifiers(abilityScores) {  //abilityScores should be an object generated by processAbilities
    //Gets the ability score modifiers from an ability score object; returns a different object with ability modifiers
    //Use this when you need integers to work with for stat conversions
...
function displayAbilities(abilityScores) { //abilityScores should be an object generated by processAbilities
    //Adds + and - symbols and ability modifiers, returns a traditional stat string
    //Use this when you want a string for users to see
    let abilityString = "";
    
    for (var ability in abilityScores) {
        let abilityPrefix = ability.substring(0,1).toUpperCase() + "" + ability.substring(1,3).toLowerCase();
        let abilityScore = abilityScores[ability];
        ...
        let abilitySubstring = abilityPrefix + " " + abilityScore + " (" + abilitySymbol + abilityModifier + ")";

        //doesn't put a space and comma before the first item in the object
        if (abilityString === "") {
            abilityString = abilitySubstring;
        }
        else {
            abilityString += ", " + abilitySubstring;
        }
