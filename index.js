...
//At this point, most of the coding stuff works and I need to work on figuring out the game design stuff.
//Need to figure out a damage conversion formula of some sort.
...
function addTextLineConditional(rawText) {  //adds a textline only if the string isn't empty
    if (rawText != "") {
        addTextLine(rawText)
    }
}
...
function getStats(monster, statName) {  //searches a "monster" array for a line that contains the relevant statName; if none, returns ""
...
function convert2e() { //converts to AD&D 2e standards
    ...
    if (monsterRanged != "") {
        ...
        monsterRanged = monsterRanged.replace(" iterative", "*")
    } 

    let monsterDefensive = getStats(monster, "Defensive Abilities");
    if (monsterDefensive === "") {
    ...
    let monsterSpecialAttacks = getStats(monster, "Special Attacks");

    let monsterWeaknesses = getStats(monster, "Weaknesses");
    if (monsterWeaknesses === "") {
        monsterWeaknesses = getStats(monster, "Weakness");
    }
    //because sometimes it's Weaknesses and sometimes it's Weakness

    let monsterSQ = getStats(monster, "SQ");

    //spells are here; doing each one as a separate entry to make it easier to change the formatting or order later
    let monsterAtWill = getStats(monster, "At will");
    let monster3Day = getStats(monster, "3/day");
    let monster2Day = getStats(monster, "2/day");
    let monster1Day = getStats(monster, "1/day");
    let monster1Week = getStats(monster, "1/week");
    let monster1Month = getStats(monster, "1/month");
    let monster1Year = getStats(monster, "1/year");
    ...
    addTextLineConditional(monsterSpecialAttacks);
    addTextLineConditional(monsterDefensive);
    addTextLineConditional(monsterSQ);
    addTextLine(""); //extra whitespace for readability
    addTextLineConditional(monsterAtWill);
    addTextLineConditional(monster3Day);
    addTextLineConditional(monster2Day);
    addTextLineConditional(monster1Day);
    addTextLineConditional(monster1Week);
    addTextLineConditional(monster1Month);
    addTextLineConditional(monster1Year); 
}