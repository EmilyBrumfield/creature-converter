...
function convert5e(newArray) {
    ...
    //sets abilityModifiers with explanation if ability scores aren't found
    let abilityModifiers = {
        Abilities: "Could not find ability scores"
    };

    //STEP ZERO: Get the important traits before messing with anything
    
    if (ifExists("Wis", newArray)) {
        ...
        abilityModifiers = getAbilityModifiers(processAbilities(targetLine));
    }
    ...
    //STEP TWO: ABILITY LINE REFORMAT
    //Grab the ability scores for later use, then reformat the line

    if (ifExists("Wis", newArray)) {
        let targetLine = getStats("Wis", newArray);
        let targetIndex = findLine("Wis", newArray);
        targetLine = displayAbilities(processAbilities(targetLine));
        newArray[targetIndex] = targetLine;
    }
